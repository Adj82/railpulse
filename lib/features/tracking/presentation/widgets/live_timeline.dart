...
class LiveTimeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TrackingBloc, TrackingState>(
      builder: (context, state) {
        final progress = state is TrackingLoaded ? state.telemetry.progress : 0.0;
              ...
              children: [
                Padding(
                  ...
                  child: ListView(
                    ...
                    children: [
                      _StationNode(name: 'New Delhi (NDLS)', time: '16:25', status: progress > 0.05 ? StationStatus.passed : StationStatus.current),
                      _buildLine(isActive: progress > 0.1),
                      _StationNode(
                        name: 'Mathura Jn', 
                        time: '18:10', 
                        status: progress < 0.25 ? StationStatus.upcoming : (progress < 0.35 ? StationStatus.current : StationStatus.passed),
                      ),
                      _buildLine(isActive: progress > 0.4),
                      _StationNode(
                        name: 'Agra Cantt', 
                        time: '19:15', 
                        status: progress < 0.5 ? StationStatus.upcoming : (progress < 0.6 ? StationStatus.current : StationStatus.passed),
                        isDelayed: true,
                      ),
                      ...
                      _StationNode(
                        name: 'Kota Jn (KOTA)', 
                        time: '21:20', 
                        status: progress < 0.9 ? StationStatus.upcoming : StationStatus.current,
                      ),
                    ],
                  ),
                ),
                ...
                Positioned(
                  left: 51,
                  top: 55 + (progress * 480).clamp(0, constraints.maxHeight - 150),
                  child: _MovingTrainIcon(speed: state is TrackingLoaded ? state.telemetry.speed.toInt() : 0),
                ),
              ],
            );
          },
  ...
  Widget _buildLine({bool isActive = false}) => Container(
    ...
    decoration: BoxDecoration(
      color: isActive ? AppColors.success.withOpacity(0.3) : Colors.black.withOpacity(0.05),
          ...
          child: Column(
            ...
            children: [
              Text(name, style: TextStyle(
                fontWeight: status == StationStatus.current ? FontWeight.w900 : FontWeight.bold,
    ...
    switch (status) {
      ...
      case StationStatus.current:
        return Container(
          ...
          child: const Center(child: Icon(Icons.circle, color: AppColors.primary, size: 14)),
        );
      case StationStatus.upcoming:
        return Container(
          width: 24, height: 24,
          decoration: BoxDecoration(border: Border.all(color: Colors.black12, width: 3), shape: BoxShape.circle, color: Colors.white),
        );
    }
  }
}
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.secondary, shape: BoxShape.circle, boxShadow: [
            BoxShadow(color: AppColors.secondary.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 6)),
          ]),
        ...
        Container(
          ...
          child: Text('$speed', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }
